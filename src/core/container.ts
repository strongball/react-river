...
export class RiverContainer {
		...
		try {
			...
			switch (kind) {
				...
				case "promiseProvider": {
					...
					const promise = override
						? (createFn(ref) as Promise<unknown>)
						: p._create(ref)
	...
	private async consumeStream(
		...
		try {
			...
			if (signal.aborted) return
			this.notifyObservers("error", provider, error)
			const current = this.getState(provider.id)?.value as
				| AsyncValue<unknown>
				| undefined
			this.updateValue(
				provider.id,
				asyncError(error, current?.data),
			)
		}
	}
	...
	private valuesEqual(a: unknown, b: unknown): boolean {
		...
		if (
			...
			return asyncValueEquals(
				a as AsyncValue<unknown>,
				b as AsyncValue<unknown>,
			)
		}
	...
	private checkAutoDispose(provider: ProviderBase): void {
		...
		const hasListeners =
			state.snapshotListeners.size > 0 || state.valueListeners.size > 0
